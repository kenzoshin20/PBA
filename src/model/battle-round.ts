...
constructor(battle: Battle, playerActions: PlayerActionEvent[]) {
    ...
    // Initialize last action times for players
    for (const player of this.battle.players) {
      this.lastActionTime[player.name] = Date.now()
    }
}
//hhshhsjs
  start() {
    ...
    // Check for AFK players before proceeding with the round
    if (this.checkForAFKPlayers()) {
      return
    }
    ...
    this.lastActionTime[action.playerName] = Date.now()
...
checkForAFKPlayers(): boolean {
    ...
    for (const player of this.battle.players) {
        const lastActionTime = this.lastActionTime[player.name]
        if (currentTime - lastActionTime > 60000) { // 1 minute in milliseconds
            this.battle.events.push({
                type: 'DISPLAY_MESSAGE',
                message: `${player.name} was inactive for too long and has been automatically conceded!`
            })
            this.battle.battleState = 'GAME_OVER'
            this.battle.winner = this.battle.getOtherPlayer(player).name
            return true
        }
    }
    return false
